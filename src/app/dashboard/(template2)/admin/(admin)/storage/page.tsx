import ComponentCard from "@/private/components/common/ComponentCard";
import PageBreadcrumb from "@/private/components/common/PageBreadCrumb";
import { Metadata } from "next";
import React from "react";

import StorageForm from "@/private/components/storage/StorageForm";

export const metadata: Metadata = {
  title: "Velog Xpress - Transport & Logistics",
  description: "Tableau de bord Velog Xpress - Storage",
    icons: {
    icon: "/favicon.png",
    shortcut: "/favicon.png",
    apple: "/favicon.png",
  },
};

export default function StorageConsultation() {
  return (
    <div>
      <PageBreadcrumb pageTitle="Storage" />
      <div className="space-y-5 sm:space-y-6">
        <ComponentCard title="Storage">
          <StorageForm />
        </ComponentCard>
      </div>
    </div>
  );
}
