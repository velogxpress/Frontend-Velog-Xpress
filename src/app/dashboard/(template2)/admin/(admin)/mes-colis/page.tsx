import ComponentCard from "@/private/components/common/ComponentCard";
import PageBreadcrumb from "@/private/components/common/PageBreadCrumb";
import { Metadata } from "next";
import React from "react";

import ColisForm from "@/private/components/mes-colis/ColisForm";

export const metadata: Metadata = {
  title: "Velog Xpress - Transport & Logistics",
  description: "Tableau de bord Velog Xpress - Colis",
    icons: {
    icon: "/favicon.png",
    shortcut: "/favicon.png",
    apple: "/favicon.png",
  },
};

export default function Colis() {
  return (
    <div>
      <PageBreadcrumb pageTitle="Colis" />
      <div className="space-y-5 sm:space-y-6">
        <ComponentCard title="Consultation de mes colis">
          <ColisForm />
        </ComponentCard>
      </div>
    </div>
  );
}
