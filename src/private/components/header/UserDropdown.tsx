...
export default function UserDropdown() {
  ...
  const handleLogout = () => {
    // 🔐 SECURITY: logout() (AuthContext) is now the single place that
    // clears every piece of session state — localStorage token, the
    // routing cookie, and cached profile fields. No need to duplicate
    // storage cleanup here (that duplication used to drift: this was the
    // only logout path that cleared the token cookie at all).
    logout();
    window.location.href = "/dashboard/signin"; // redirect to login page
  };
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="block mr-1 font-medium text-theme-sm">{clientName|| "User"}</span>
      ...
      >
        <div>
          <span className="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">
            {cleanVelogCode(clientCode) || "User"}</span>
