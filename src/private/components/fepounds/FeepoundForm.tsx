...
export default function FeepoundForm() {
  ...
  const [recherche, setRecherche] = useState("");
    const [editingFeepound, setEditingFeepound] = useState<Feepounds | null>(null);
  ...
  const handleKeyUp = async () => {
    if (recherche.trim() === "") {
      ...
      try {
        const response = await getFeepoundByAmount(recherche, page);
        setFeepounds(response.data.content);
        setTotalPages(response.data.totalPages);
      } catch (error) {
  ...
  function handleOpenEditModal(selectedFeepound: Feepounds) {
    setEditingId(selectedFeepound.id);
    setEditingFeepound(selectedFeepound);
    openEditModal();
  }

   function handleOpenDeleteModal(selectedFeepound: Feepounds) {
     setEditingId(selectedFeepound.id);
     setEditingFeepound(selectedFeepound);
     openDeleteModal();
   }
  ...
  function handleSafeDelete(): void {
    ...
    deleteFeepound(editingId)
      .then(() => {
        setFeepounds((prev) => prev.filter((v) => v.id !== editingId));
        // If the current page becomes empty after deletion, refetch previous page if possible.
        setTimeout(() => {
          setEditingId(null);
          setEditingFeepound(null);
          closeDeleteModal();
          setFeepounds((current) => {
            if (current.length === 0 && page > 0) {
              const newPage = page - 1;
              setPage(newPage);
              fetchFeepounds(newPage);
            } else {
              fetchFeepounds(page);
            }
            return current;
  ...
  useEffect(() => {
   ...
   async function handleEditSave(e: React.FormEvent) {
     e.preventDefault();
     if (!editingFeepound) return;

     const payload = {
       amount: editingFeepound.amount,

     };

     try {
       await updateFeepound(editingId!, payload);
       closeEditModal();
       fetchFeepounds(page);
     } catch (e) {
       console.error(e);
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                      <div className="flex items-center gap-3">
                        <Button
                          ...
                          className="p-2"
                          variant="outline"
                          onClick={() => handleOpenEditModal(feepound)}
                        >
                        ...
                        <Button
                          ...
                          className="p-2"
                          variant="outline"
                          onClick={() => handleOpenDeleteModal(feepound)}
                        >
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Créer frais par livre
            </h4>
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Enregistrer un nouveau frais par livre pour l&apos;utiliser dans vos
              opérations.
            </p>
          </div>
              ...
              <div>
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-1">
                  <div>
                    ...
                    <Input
                      type="text"
                      value={editingFeepound?.amount || ""}
                      onChange={(e) =>
                        setEditingFeepound((prev) => ({
                          ...prev!,
      ...
      >
        <div className="no-scrollbar relative w-full max-w-[450px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          ...
          <div className="px-2  text-center">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Êtes-vous sûr de vouloir supprimer ce frais par livre?
            </p>
          </div>
